...
def battery_percent_at_or_before(seconds_ago):
    if not mode_percent_history:
        return None
    ...
    return reference.get("percent") if reference else None
...
def build_vertical_battery_panel(data):
    ...
    ghost_percent = battery_percent_at_or_before(600)
    ...
    trend_text = Text()
    if delta_10m is None:
        trend_text.append("10m: N/A", style="dim")
    else:
        trend_style = "green" if delta_10m > 0 else "yellow" if delta_10m < 0 else "white"
        trend_text.append(f"10m: {delta_10m:+.2f}%", style=trend_style)
