# BatteryTop_v9_2_AlertsEventsWarnings.py
...
def build_battery_panel(data):
    ...
    return Panel(table, title="BatteryTop V9.2 - Dell")
...
def build_mode_percent_timeline_panel(data):
    ...
    stats.add_column(width=18, style="cyan")
    stats.add_column()
    stats.add_row("Mode line", Text("green=charge  yellow=discharge  dim=idle"))
    stats.add_row("Percent line", Text("block height = battery percentage"))
    stats.add_row("Samples", f"{len(valid)} total, {len(samples)} shown")
...
def build_dashboard_layout(data):
    ...
    # Target grid:
    # Row 1: BatteryTop | Session Analytics + Charge Session Log
    # Row 2: Battery Health | Power Monitor + Alerts & Events
    # Row 3: Battery Charge full width
    # Row 4: Battery Mode + Percent Timeline full width
    # Row 5: Charge Power History | Discharge Power History
    layout.split_column(
        Layout(name="row_status", ratio=5, minimum_size=12),
        Layout(name="row_health", ratio=4, minimum_size=10),
        Layout(name="row_progress", size=3),
    ...
    layout["row_progress"].update(build_main_progress(data))
